...
import { useRoqClient, useEnrollmentFindManyWithCount } from 'lib/roq';
...
export function EnrollmentListPage(props: EnrollmentListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useEnrollmentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.enrollment.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.completion_date ? format(record?.completion_date, 'dd-MM-yyyy') : '',
    },
