...
import { useRoqClient, useAssignmentFindManyWithCount } from 'lib/roq';
...
export function AssignmentListPage(props: AssignmentListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useAssignmentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.assignment.delete({ where: { id } });
      await mutate();
    } catch (error) {
